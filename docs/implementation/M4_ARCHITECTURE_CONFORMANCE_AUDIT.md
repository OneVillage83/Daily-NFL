...
| M4-16 | Fuzzy matching never silently resolves | `SATISFIED` | Review-only fuzzy candidates remain non-selecting; DB fuzzy crosswalk trigger retained. |
| M4-17 | Ambiguous candidate sets remain unresolved | `SATISFIED` | Multiple game/drive/play candidates produce `AMBIGUOUS` with no selected identity. |
| M4-18 | Unresolved decisions retain candidates and confidence | `SATISFIED` | Existing decision/candidate contract retained. |
| M4-19 | Unresolved decisions retain source row/raw evidence | `SATISFIED AFTER REMEDIATION` | Added typed `ReconciliationEvidence` and append-only relational evidence ledger linked to M3 raw content/acquisition observations. |
| M4-20 | Matching facts used by reconciliation are auditable | `SATISFIED AFTER REMEDIATION` | Evidence rows persist structured `facts_json` rather than prose-only rationale. |
| M4-21 | New crosswalks cite a reconciliation decision | `SATISFIED AFTER REMEDIATION` | Migration v6 requires non-null decision ID and an existing decision row for new crosswalk inserts. |
| M4-22 | Resolved decision + crosswalk write is atomic | `SATISFIED AFTER REMEDIATION` | Repository savepoint records decision/evidence and binding as one unit; failed binding removes provisional decision rows. |
| M4-23 | Crosswalk history is append-only | `SATISFIED` | Existing update/delete triggers retained. |
| M4-24 | Reconciliation decision history is append-only | `SATISFIED` | Existing decision update/delete triggers retained. |
| M4-25 | Reconciliation evidence history is append-only | `SATISFIED AFTER REMEDIATION` | Migration v6 adds update/delete rejection triggers. |
| M4-26 | Verification/confidence/method change cannot be silently collapsed | `SATISFIED AFTER REMEDIATION` | Same target/interval with changed resolution metadata requires explicit supersession. |
| M4-27 | Supersession preserves earlier historical mapping | `SATISFIED` | Existing supersedes relationship retained and index widened for explicit metadata revisions. |
| M4-28 | Forward migration preserves M3/M4 provisional history | `STATIC TEST IMPLEMENTED; LOCAL GATE PENDING` | v5→v6 migration test retains legacy decision/crosswalk rows. |
| M4-29 | M1/M2/M3 certified contracts remain intact | `STATIC REVIEW COMPLETE; LOCAL GATE PENDING` | Migration is additive/forward-only; no rewrite of migrations 1–5. |
| M4-30 | Real small provider sample reaches canonical reconciliation | `LOCAL REAL-PROVIDER GATE PENDING` | `scripts/validate_m4_reconciliation.py`. |
| M4-31 | Full repository quality gate | `LOCAL VALIDATION PENDING` | pytest / Ruff / strict mypy / clean tree required. |
...
### F-04 — reconciliation evidence was not atomically coupled to binding
...
### F-05 — crosswalk metadata changes could be silently collapsed
...
### F-06 — database did not require crosswalk decision provenance
...
### F-07 — reconciliation identity vocabulary lagged the certified domain vocabulary
...
- overlapping active mappings to different canonical identities fail;
