...
- [`PENALTY_EVENT_AND_DISCIPLINE_ARCHITECTURE_V1.md`](./PENALTY_EVENT_AND_DISCIPLINE_ARCHITECTURE_V1.md) — governing extension to F-5/F-6/F-13 for first-class penalty truth, partial `no_play` penalty-event retention, physical-vs-official outcome separation, offending-team cost vs beneficiary value, discipline state, future penalty feature families, leverage/EPA/WPA plans, PIT rules, and M6/M7/M9+ responsibilities.
...
The Daily NFL architecture is now complete through **F-24**, with versioned governing extensions where real-data validation identifies an implementation requirement that should be carried forward:

```text
LAYER 1 — TRUTH & EVIDENCE
F-0 → F-5
  └── Penalty Event & Discipline Architecture V1
